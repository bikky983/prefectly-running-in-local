...
try:
    ...
    # dotenv not available, will use environment variables directly (GitHub Actions)
    logger = logging.getLogger(__name__)
    logger.info("🔧 Using environment variables directly (GitHub Actions mode)")
...
class SocialMediaPoster:
    ...
    def __init__(self):
        ...
        logger.info(f"🔑 Facebook Token: {'✅ Found' if self.fb_access_token else '❌ Missing'}")
        logger.info(f"🔑 Facebook Page ID: {'✅ Found' if self.fb_page_id else '❌ Missing'}")
        logger.info(f"🔑 Instagram Token: {'✅ Found' if self.ig_access_token else '❌ Missing'}")
        logger.info(f"🔑 Instagram User ID: {'✅ Found' if self.ig_user_id else '❌ Missing'}")
